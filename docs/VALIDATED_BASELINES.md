# Validated baselines before Gate 2C

Last updated: 2026-08-14.

This document records facts measured or hardware-validated during recent RAVE
development. It does **not** make the repository road-ready or production-ready.
Raspberry Pi 5 runtime performance, the production metadata protocol/receiver,
authenticated transport, end-to-end fault handling, and vehicle validation remain
open work.
...
- current live-inference baseline software crop: 1920x391, `y=300:691`;
- YOLO input size: 960;
- confidence threshold used for the validated baseline: 0.05;
...
operation after the capture/inference pipeline fixes. This is an x86/NVIDIA
development baseline, **not** Pi 5 validation.
...
During the latest validation, the generated RAVE NetworkManager profile UUID was
...
integration groundwork. This is not yet the RAVE perception-metadata receiver.
...
The UI contract validated for Gate 2B is intentionally thin:
...
- Pi 5 + accelerator inference/tracking latency, thermals, power, USB load, and frame
  age under the actual deployment stack;
- production crop/danger-zone determination and temporal model validation;
